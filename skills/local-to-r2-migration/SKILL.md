...
**Problem:** The local filesystem has accumulated thousands of files that should live on Cloudflare R2. Agents pulled files from R2 and never cleaned them up. Orphaned `_*` ephemeral files, git objects, build artifacts, and entire project directories clutter `G:\My Drive\projects\` and `G:\My Drive\QWAV\`.
...
**Thin-Client Mandate:** The ONLY files that should persist locally are the import surface (`G:\My Drive\prompts\`). Everything else lives on R2. This skill is the enforcement mechanism.
...
| **IMPORT-SURFACE** | Inside `G:\My Drive\prompts\` | SKIP — this is the canonical local surface |
...
python _migration_scanner.py --scan "G:/My Drive/projects" --scan "G:/My Drive/QWAV" --output _migration_report.json
...
```json
{
  ...
  "targets": ["G:/My Drive/projects", "G:/My Drive/QWAV"],
  "totals": {
    "files_found": 11072,
    "total_size_mb": 0.0
  },
...
Get-ChildItem -Recurse -File -Path "G:/My Drive/projects", "G:/My Drive/QWAV" |
...
Get-ChildItem -Recurse -Directory -Path "G:/My Drive/projects", "G:/My Drive/QWAV" |
...
$remaining = Get-ChildItem -Recurse -File -Path "G:/My Drive/projects", "G:/My Drive/QWAV" |
...
- `G:/My Drive/projects/<project>/<file>` → `qnfo/projects/<project>/<file>`
- `G:/My Drive/QWAV/<path>` → `qnfo/qwav/<path>` (skip `.git/` directory)

For standalone files at the top level of projects/:
- `G:/My Drive/projects/<file>` → `qnfo/projects/_top/<file>`

**Step 3.2: Batch upload with verification**

```bash
# For each migration candidate, upload and verify:
npx wrangler r2 object put qnfo/projects/<project>/<path> --file="G:/My Drive/projects/<project>/<path>" --remote
...
{
  "uploaded": [
    {"local": "G:/My Drive/projects/x/file.md", "r2": "qnfo/projects/x/file.md", "verified": true}
  ],
  "skipped_conflicts": [],
  "failed": []
}
...
for root, dirs, files in os.walk('G:/My Drive/projects', topdown=False):
    if root != 'G:/My Drive/projects' and not files and not dirs:
        try:
            os.rmdir(root)
...
Write-Output "projects/: $((Get-ChildItem -Recurse -File -Path 'G:/My Drive/projects' | Where-Object { $_.DirectoryName -notmatch '\\.wrangler|__pycache__|\\.git' }).Count) files remain"
Write-Output "QWAV/: $((Get-ChildItem -Recurse -File -Path 'G:/My Drive/QWAV' | Where-Object { $_.DirectoryName -notmatch '\\.wrangler|__pycache__|\\.git' }).Count) files remain"
...
|:-------|:---------------|:--------|
| `migration_scanner.py` | `G:\My Drive\prompts\tools\migration_scanner.py` | File classification, report generation |

### Bootstrap Protocol

```bash
Test-Path "G:\My Drive\prompts\tools\migration_scanner.py"
# If MISSING: create from embedded source below
# After creation: upload to R2 for future use
# npx wrangler r2 object put qnfo/tools/migration_scanner.py --file="G:\My Drive\prompts\tools\migration_scanner.py" --remote
```
...
read('G:\My Drive\prompts\skills\knowledge-graph\SKILL.md')
...
Agent:
  1. [Phase 1] Scans G:/My Drive/projects/ and G:/My Drive/QWAV/
