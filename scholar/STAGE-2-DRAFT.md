...
- NO bare Unicode math characters in ANY manuscript output (Greek, operators, blackboard bold, sub/superscripts).
- ALL mathematical content must use $...$ (inline) or $$...$$ (display) with proper LaTeX commands.
- Before delivering the draft, scan for bare Unicode math outside $...$/$$...$$/code blocks and convert to LaTeX.
- Code blocks are exempt from math formatting.
- If bare Unicode math is detected in the draft, fix it before marking the stage complete.
...
**Step 2.2: Septenary Protocol (nested implementation)**
...
```json
{
  "S4_EVIDENCE_LEDGER": {
    "meta": {
      "timestamp": "[ISO 8601]",
      "agent_version": "research_draft_v1.0",
