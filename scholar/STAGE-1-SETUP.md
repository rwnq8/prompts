...
**RELEASE PUBLISHING RULE:** This stage (Setup) does NOT write to `R2 releases (qnfo/releases/)\`. Publication happens ONLY through STAGE-4, which requires explicit user approval. Do not autonomously place files in the releases directory.
...
- **Paper not on arXiv (journal-only, no preprint):** Search by DOI or title + authors via `brave_web_search` for publisher page, Semantic Scholar, or author's institutional repository. If HTML full-text is accessible: use `load_url` + `cdp_send` for structured extraction (same protocol, different base URL). If only PDF is available: flag as `[PDF-ONLY: <paper ID> — no structured HTML extraction possible]`. If fully paywalled with no open access: flag as `[UNAVAILABLE: <paper ID> — no accessible version found]`.
