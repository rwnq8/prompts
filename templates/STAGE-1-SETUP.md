...
1. **Code Supremacy:** Python execution is the ONLY valid source of quantitative results.
2. **Source Traceability:** Every factual claim must be traceable to an external source file OR Python code execution.
3. **Citation Integrity:** Citations must reference external source files present in the project directory.
...
> **WARNING ERROR HANDLING:** All gh commands in this stage inherit the retry strategy from QWAV-DEFAULT.md Section 0.9.1. Every gh command retries up to 3x with exponential backoff (1s, 4s, 16s) for transient failures.
...
**EXECUTION MODE:** ANALYTICAL to SOURCE VERIFICATION to ARCHITECTURAL
...
3. Build the source catalog from these file-backed sources
...
1. **First, attempt autonomous web search:** Use `brave_web_search` to find relevant literature, papers, and sources on the defined domain. Label all results `[WEB-SEARCH]` with query and timestamp.
...
3. Output this manifest with instruction to execute searches externally. **PAUSE** — do not fabricate citations.
...
**Step 2.4: Literature Landscape Analysis**
200-300 words:
...
### PHASE 2.5: DEEP-READ PROTOCOL FOR PUBLISHED PAPERS

**WHEN TO USE:** When the research requires deep-reading specific published papers (arXiv preprints, journal articles, conference proceedings). This protocol applies to ANY research domain.
...
2. Use `cdp_send` with `Runtime.evaluate` for STRUCTURED extraction:
   - Abstract, sections by heading, equations, references
3. Save ALL extracted text to a versioned local file: `<paper_id>_text.txt`
4. **CRITICAL:** Parse the bibliography to identify cited papers requiring follow-up retrieval
...
- **arXiv search returns empty:** Retry with broader keywords. After 3 failed attempts: `[RETRIEVAL-FAILED: <paper ID>]`. Continue with available sources.
- **brave_web_search rate-limited:** Wait 60 seconds, retry once. If still rate-limited: `[WEB-SEARCH-FAILED: rate-limit]`.
- **PDF-only papers (no arXiv HTML):** Extract abstract + metadata. NEVER cite claims from abstract-only access as authoritative.
- **Paywalled papers:** Try arXiv preprint, author's institutional page, ResearchGate. After exhausting alternatives: `[FULL-TEXT-UNAVAILABLE: paywall]`.
- **Paper not on arXiv (journal-only):** Search by DOI or title via `brave_web_search` for publisher page or Semantic Scholar.

**YoBrowser extraction failures:**
- **Browser timeout (>=30 seconds):** Kill session, restart with fresh `load_url`. After 3 retries: `[BROWSER-TIMEOUT: <URL>]`.
- **ArXiv HTML page malformed:** Try alternative selectors. If still fails: save raw HTML, extract text via Python regex.
- **Non-standard paper structure:** Extract all available content. Flag: `[NON-STANDARD-STRUCTURE]`.
- **Math/equation extraction failure:** Extract raw LaTeX from source. Flag: `[MATH-EXTRACTION-PARTIAL]`.

**Python extraction failures:**
- **Python execution error during text processing:** Debug and retry up to 3 times. If persistent: `[PYTHON-FAILURE]`.
- **Encoding issues (Windows cp1252):** Re-extract with explicit UTF-8 encoding, use `errors='replace'` mode.

### PHASE 3: STRUCTURAL ARCHITECTURE

**Step 3.1: Gap Analysis Construction**
...
Standard academic structure (Introduction, Methods, Results, Discussion) or domain-adapted:
- Introduction (2-3 subsections) to Literature Review (3-4) to Methodology (3-4) to Results (3-4) to Discussion (3-4) to Conclusion (2-3) to Future Work (1-2)
...
**No source files available:** Use `brave_web_search` to discover sources autonomously, or generate external search request.
**Insufficient sources (<10):** Flag `insufficient_sources_warning`. Prioritize quality over quantity.
**Source files with incomplete metadata:** Extract whatever is available. Flag missing fields.
**Unreadable source files:** Log as `[UNREADABLE-FILE: <path>]`. If >=50% unreadable, escalate to `[BLOCKED]`.

---

*STAGE-1-SETUP v6.1 — Research Planning Agent | First of 4-stage pipeline*
